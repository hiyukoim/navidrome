...
var _ = Describe("lastfmAgent", func() {
	...
	Describe("GetArtistBiography", func() {
		...
		BeforeEach(func() {
			...
			client := newClient("API_KEY", "SECRET", "https://ws.audioscrobbler.com/2.0/", httpClient)
			agent = lastFMConstructor(ds)
			agent.client = client
		})
	...
	Describe("Language Fallback", func() {
		Describe("GetArtistBiography", func() {
			...
			It("returns content in first language when available (1 API call)", func() {
				conf.Server.LastFM.Languages = []string{"pt", "en"}
				agent = lastFMConstructor(ds)
				agent.client = newClient("API_KEY", "SECRET", "https://ws.audioscrobbler.com/2.0/", httpClient)
	...
	Describe("GetArtistImages", func() {
		...
		BeforeEach(func() {
			...
			httpClient = &tests.FakeHttpClient{}
			client := newClient("API_KEY", "SECRET", "https://ws.audioscrobbler.com/2.0/", apiClient)
			agent = lastFMConstructor(ds)
			agent.client = client
