...
var _ = Describe("client", func() {
	...
	BeforeEach(func() {
		...
		client = newClient("API_KEY", "SECRET", consts.DefaultLastFMBaseURL, httpClient)
	})
	...
	Describe("normalizeLastFMBaseURL", func() {
		It("returns the default when empty", func() {
			Expect(normalizeLastFMBaseURL("")).To(Equal(consts.DefaultLastFMBaseURL))
			Expect(normalizeLastFMBaseURL("   ")).To(Equal(consts.DefaultLastFMBaseURL))
		})
		It("appends a trailing slash when missing", func() {
			Expect(normalizeLastFMBaseURL("http://proxy:8080")).To(Equal("http://proxy:8080/"))
		})
		It("keeps an existing trailing slash", func() {
			Expect(normalizeLastFMBaseURL("http://proxy:8080/")).To(Equal("http://proxy:8080/"))
		})
	})

	Describe("makeRequest with custom BaseURL", func() {
		It("posts to the configured BaseURL", func() {
			httpClient.Res = http.Response{Body: io.NopCloser(bytes.NewBufferString(`{}`)), StatusCode: 200}
			custom := newClient("API_KEY", "SECRET", "http://lastfm-alias-proxy:8080", httpClient)
			err := custom.updateNowPlaying(context.Background(), "SESSION_KEY", ScrobbleInfo{
				artist: "U2",
				track:  "One",
			})
			Expect(err).To(BeNil())
			Expect(httpClient.SavedRequest.URL.String()).To(Equal("http://lastfm-alias-proxy:8080/"))
		})
	})
})
